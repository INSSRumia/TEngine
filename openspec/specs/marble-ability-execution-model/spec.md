...
Marble 战斗域 MUST 提供显式结算触发点，用于在状态变化后触发离散逻辑，例如伤害结算、死亡判定和升级检测。此类逻辑 MUST 不依赖逐帧被动轮询才能生效。Marble 的配置驱动能力装配 ALSO MUST 通过统一的配置挂载入口完成，不得将配置解析逻辑分散到多个彼此无关的装配路径中。Marble abilities 对 RuntimeData 的访问 ALSO MUST 通过结构化黑板完成，使状态、配置和帧临时数据的访问边界清晰可见。
...
#### Scenario: Ability reads structured runtime blackboard
- **WHEN** Marble Ability 读取或写入运行时黑板
- **THEN** 它必须能够明确区分状态数据、配置投影数据和帧临时数据
- **AND** 不应继续依赖单层平铺字段才能完成访问
