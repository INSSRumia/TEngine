...
战斗域中的 `MarbleFactory`、`EquipmentFactory`、`ProjectileFactory` 在装配能力时 MUST 使用一致的结构化骨架，至少明确区分固定骨架能力挂载、配置驱动能力挂载和配置 creator 扩展点三部分。Factory 在装配过程中 ALSO MUST 将配置对象直接传递给 `RuntimeData` 与 `Ability` 构造，而不是在 Factory 中长期展开配置字段细节。
...
每个 Combat Factory 对于配置驱动能力 MUST 提供单一且显式的挂载路径，配置能力的创建、判空、优先级设置和最终挂载顺序必须可预测。该路径 ALSO MUST 以“传递配置对象给能力实例”为默认模式，而不是在工厂层展开配置字段后再拼接构造参数。
...
#### Scenario: Factory forwards config object instead of exploded scalar list
- **WHEN** 工厂或默认 creator 创建依赖配置字段的能力实例
- **THEN** 工厂必须直接传入对应配置对象
- **AND** 不得继续在 Factory 主路径中出现成组的 `config.xxx.yyy` 标量展开构造
